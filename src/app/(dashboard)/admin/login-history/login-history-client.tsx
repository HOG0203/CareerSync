...
export function LoginHistoryClient({ logs }: LoginHistoryClientProps) {
  ...
  const formatTargetName = (targetName?: string) => {
    ...
    return targetName
      ...
      .replace(/admin\/students/g, '학생 등록/진급')
      .replace(/\[admin\/student-accounts\]/g, '[학생 계정 관리]')
      .replace(/admin\/student-accounts/g, '학생 계정 관리')
      .replace(/\[student-accounts\]/g, '[학생 계정 관리]')
      .replace(/student-accounts/g, '학생 계정 관리')
      .replace(/\[merit-demerit\]/g, '[상벌점 관리]')
      .replace(/merit-demerit/g, '상벌점 관리')
      .replace(/\[student\/merit-demerit\]/g, '[학생 상벌점 내역 조회]')
      .replace(/student\/merit-demerit/g, '학생 상벌점 내역 조회')
      .replace(/\[student\/certification\]/g, '[학생 옥저인재인증 평가표]')
      .replace(/student\/certification/g, '학생 옥저인재인증 평가표')
      .replace(/\[employment-status\]/g, '[취업진로현황]')
      .replace(/employment-status/g, '취업진로현황')
      .replace(/\[field-training\]/g, '[현장실습/도제OJT현황]')
      .replace(/field-training/g, '현장실습/도제OJT현황')
      .replace(/\[company-info\]/g, '[업체정보]')
      .replace(/company-info/g, '업체정보')
      .replace(/\[labor-education\]/g, '[노동인권교육]')
      .replace(/labor-education/g, '노동인권교육')
      .replace(/\[class-management\]/g, '[학반 관리]')
      .replace(/class-management/g, '학반 관리')
      .replace(/\[students\]/g, '[취업상세데이터]')
      .replace(/students/g, '취업상세데이터')
      .replace(/\[dashboard\]/g, '[대시보드]')
      .replace(/dashboard/g, '대시보드');
  };
