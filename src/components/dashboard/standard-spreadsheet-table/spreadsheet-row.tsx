...
export const SpreadsheetRow = React.memo(({ row, rIdx, columns, selMinR, selMaxR, selMinC, selMaxC, selStart, editCell, onMouseDown, onMouseEnter, onStartEdit, onEndEdit, onSave, isSelectedRow, onSelectRow, onAction, rankingMap, isRankingsLoading, userProfile, disableNamePopover, baseYear, hideCheckbox, masterCompanies }: any) => {
  ...
  return (
    <tr className={cn("h-8 transition-none hover:bg-slate-50/50", isSelectedRow && "bg-blue-50/30")}>
      ...
      {columns.map((col: any, cIdx: number) => (
        <SpreadsheetCell key={col.key} id={row.id} field={col.key} value={row[col.key]} config={col} rowData={row} rIdx={rIdx} cIdx={cIdx} isSelected={isRowInSelection && cIdx >= selMinC && cIdx <= selMaxC} isFocused={selStart?.row === rIdx && selStart?.col === cIdx} isEditing={editCell?.row === rIdx && editCell?.col === cIdx} onMouseDown={(m: any) => onMouseDown(rIdx, cIdx, m)} onMouseEnter={() => onMouseEnter(rIdx, cIdx)} onStartEdit={() => onStartEdit(rIdx, cIdx)} onEndEdit={onEndEdit} onSave={onSave} onAction={onAction} rankingMap={rankingMap} isRankingsLoading={isRankingsLoading} userProfile={userProfile} disableNamePopover={disableNamePopover} baseYear={baseYear} masterCompanies={masterCompanies} />
      ))}
    </tr>
  );
}, (p, n) => {
  if (p.rIdx !== n.rIdx || p.row !== n.row || p.isSelectedRow !== n.isSelectedRow || p.rankingMap !== n.rankingMap || p.isRankingsLoading !== n.isRankingsLoading || p.userProfile !== n.userProfile || p.disableNamePopover !== n.disableNamePopover || p.baseYear !== n.baseYear || p.hideCheckbox !== n.hideCheckbox || p.masterCompanies !== n.masterCompanies) return false;
