...
/**
 * 성적/출결/자격증 현황 전용 필터 및 데이터 스켈레톤 UI
 */
    ...
    <div className="flex flex-col h-full gap-4 sm:gap-6 animate-pulse p-1">
      ...
      <div className="md:hidden space-y-3">
        {Array.from({ length: 5 }).map((_, i) => (
          <div key={i} className="p-4 bg-white rounded-2xl border border-slate-200/80 space-y-3 shadow-sm">
      ...
      <div className="hidden md:block flex-1 bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden p-4 space-y-3">
        <div className="h-10 bg-slate-100 rounded-xl w-full" />
        {Array.from({ length: 7 }).map((_, i) => (
          <div key={i} className="h-12 bg-slate-50/80 rounded-lg w-full flex items-center px-4 justify-between gap-4 border border-slate-100">
