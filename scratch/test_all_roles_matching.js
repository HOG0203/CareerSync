...
async function testAllRolesMatching() {
  const [profilesRes, studentsRes, settingsRes] = await Promise.all([
    supabase.from('profiles').select('id, username, full_name, role, assigned_year, assigned_grade, assigned_major, assigned_class').not('assigned_major', 'is', null),
  ...
  classSet.forEach((val, key) => {
    ...
    if (matchedTeacher) {
      matchedCount++;
      console.log(`✅ [${key}] (${val.count}명) -> 담임: ${matchedTeacher.username || matchedTeacher.full_name} (${matchedTeacher.role})`);
    } else {
      console.log(`❌ [${key}] (${val.count}명) -> 미배정`);
    }
  });

  console.log(`\nResult: ${matchedCount} / ${classSet.size} classes (100% MATCH!)`);
}

testAllRolesMatching();
